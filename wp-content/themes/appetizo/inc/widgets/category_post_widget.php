...
add_action( 'widgets_init', 'appetizo_latest_news_load_widget' );

function appetizo_latest_news_load_widget() {
    register_widget( 'appetizo_latest_news_widget' );
}

class appetizo_latest_news_widget extends WP_Widget {
    ...
    public function __construct() {
        /* Widget settings. */
        $widget_ops = array( 'classname' => 'appetizo_latest_news_widget', 'description' => __('A post widget that can display your latest posts, posts from a category, or hand-picked posts by ID.', 'appetizo') );

        /* Widget control settings. */
        $control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'appetizo_latest_news_widget' );

        /* Create the widget. */
        parent::__construct( 'appetizo_latest_news_widget', __('Appetizo: Post Widget', 'appetizo'), $widget_ops, $control_ops );
    }
                ...
                <div class="side-pop <?php if($layout == 'small_thumb') : ?>list<?php endif; ?>">
                    ...
                    <div class="side-pop-img">
                        <?php if(has_post_thumbnail()) : ?>
                                ...
                                <a href="<?php the_permalink() ?>"><?php the_post_thumbnail('appetizo-widget-small-thumb'); ?></a>

                        <?php else : ?>

                            <a href="<?php the_permalink() ?>">
                                <img src="<?php echo esc_url(get_template_directory_uri() . '/images/slider-default.png'); ?>" alt="<?php esc_attr_e('Default', 'appetizo'); ?>" /></a>
    ...
    function form( $instance ) {
        ...
        $defaults = array( 'title' => __('Latest Posts', 'appetizo'), 'number' => 3, 'categories' => '', 'layout' => '', 'date' => false, 'counter' => false, 'postids' => '');
        ...
        <!-- Layout -->
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('layout')); ?>"><?php esc_html_e( 'Choose Layout', 'appetizo' ); ?>:</label>
            <select id="<?php echo esc_attr($this->get_field_id('layout')); ?>" name="<?php echo esc_attr($this->get_field_name('layout')); ?>" class="widefat categories" style="width:100%;">
                <option value='small_thumb' <?php if ('small_thumb' == $instance['layout']) echo 'selected="selected"'; ?>><?php esc_html_e( 'Small Thumbnail', 'appetizo' ); ?></option>
                <option value='text_layout' <?php if ('text_layout' == $instance['layout']) echo 'selected="selected"'; ?>><?php esc_html_e( 'Text Only List', 'appetizo' ); ?></option>
            </select>
        </p>

        <!-- Category -->
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('categories')); ?>"><?php esc_html_e( 'Filter by Category', 'appetizo' ); ?>:</label>
            <select id="<?php echo esc_attr($this->get_field_id('categories')); ?>" name="<?php echo esc_attr($this->get_field_name('categories')); ?>" class="widefat categories" style="width:100%;">
                <option value='all' <?php if ('all' == $instance['categories']) echo 'selected="selected"'; ?>><?php esc_html_e('All categories', 'appetizo'); ?></option>
        ...
        <!-- Post IDs -->
        <p>
            <label for="<?php echo esc_attr($this->get_field_id( 'postids' )); ?>"><?php esc_html_e('Post IDs (separate with comma):', 'appetizo'); ?></label>
            ...
            <small><?php esc_html_e( 'Display specific posts using post IDs. This option will override the category option above.', 'appetizo' ); ?></small>
        </p>

        <!-- Number of posts -->
        <p>
            <label for="<?php echo esc_attr($this->get_field_id( 'number' )); ?>"><?php esc_html_e('Number of posts to show:', 'appetizo'); ?></label>
        ...
        <!-- Numbering -->
        <p>
            <label for="<?php echo esc_attr($this->get_field_id( 'counter' )); ?>"><?php esc_html_e( 'Add Numbering', 'appetizo' ); ?>:</label>
        ...
        <!-- Date -->
        <p>
            <label for="<?php echo esc_attr($this->get_field_id( 'date' )); ?>"><?php esc_html_e( 'Hide Date', 'appetizo' ); ?>:</label>
